<x-app-layout>
    ...
    @section('konten')
        <div class="container mt-5" style="background-color: white; width: 400px; border-radius: 20px; height: 70px;  ">
            ...
            <form method="post" action="{{ route('pelanggan.profile.update') }}" class="mt-6 space-y-6">
                @csrf
                @method('patch')
                ...
                <div class="form-group mt-4">
                    <div class="d-flex col-sm-offset-2 col-sm-12" style="justify-content: right">
                        <button type="submit" class="btn w3-indigo"
                            style="background-color: #135589 !important; border-radius: 20px; width: 116px; margin :10px 0 25px ">{{ __('Simpan') }}</button>
                    </div>
                </div>
            </form>
        </div>
